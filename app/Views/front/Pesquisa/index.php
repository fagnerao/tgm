  ...
  <div class="max-w-2xl mx-auto">
        <!-- search list form -->
            <form class="flex items-center mb-8" method="POST" >   
                ...
                <a href="<?= base_url('/pesquisa') ?>" class="p-2.5 ml-2 text-sm font-medium text-gray-900 bg-gray-200 rounded-lg border border-gray-700 hover:bg-gray-400 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">
...
if (!empty($resultado)){  ?>
    <div class="relative">
      ...
      <h3 class="bb-2 text-lg uppercase text-center">Notícias e Comunicados</h3>
     <?= ($pesquisa)? '<h3 class="font-sans mb-4 text-green-500"> Resultado da pesquisa :<strong> '. $pesquisa.'</strong></h3>': '' ?>
            ...
            <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                ...
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="<?= ($rst->id % 2 ==0)?'':'bg-gray-50' ?>">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-lg"><a href="<?= base_url('pagina/'.$rst->slug)?>" class="text-green-800"><i class="ri-arrow-right-circle-fill mt-2 pr-3"></i> <?= $rst->titulo ?></a></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= character_limiter($rst->texto, 100) ?> </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= dateTimeBr($rst->updated_at) ?> </td>
                        </tr>
                        <?php } ?>
                    </tbody>
                    </table>
        ...
        <?php } else {?>
        <h1 class="text-center font-semibold text-2xl">Nenhum texto encontrado</h1>
        <?= ($pesquisa)? '<h3 class="font-sans mb-4 text-green-500"> Resultado da pesquisa :<strong> '. $pesquisa.'</strong></h3>': '' ?>
        <?php } ?>
  </div>
</div>
