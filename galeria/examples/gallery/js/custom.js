$(document).ready(function() {
    ...
    var $fileuploader = $('input.gallery_media').fileuploader({
        limit: 100,
        fileMaxSize: 20,
        extensions: ['image/*', 'jpg', 'png', 'gif', 'webp', 'jpeg', 'pdf', 'video/*', 'audio/*'],
        changeInput: ' ',
        theme: 'gallery',
        enableApi: true,
        thumbnails: {
        ...
        upload: {
            ...
            url: url_site + '/galeria/examples/gallery/php/ajax.php?type=upload',
            data: null,
            type: 'POST',
            ...
            beforeSend: function(item) {
                ...
                item.upload.data.page_id = $('.gallery_media').attr('data-page-id');
        ...
        editor: {
            ...
            onSave: function(dataURL, item) {
                ...
                if (item.appended && item.data.listProps) {
                    ...
                    $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=resize', { name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor) }, function() {
                        // update the image
        ...
        sorter: {
            onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
                ...
                // send request
                $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=sort', {
            ...
            $('body').on('click', function(e) {
                ...
                if ($target.is('.gallery-action-rename')) {
                    ...
                    if (x && item.data.listProps) {
                        $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=rename', { name: item.name, id: item.data.listProps.id, title: x }, function(result) {
                            try {
                ...
                if ($target.is('.gallery-action-asmain') && item.data.listProps) {
                    ...
                    var page_id = $('.gallery_media').attr('data-page-id');

                    //console.log(item.data.listProps.page_id);
                    $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=asmain', { name: item.name, page_id: page_id, id: item.data.listProps.id }, function() {
        ...
        onRemove: function(item) {
            // send request
            if (item.data.listProps)
                $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=remove', {
                    name: item.name,
                    id: item.data.listProps.id
                });
        },
        captions: $.extend(true, {}, $.fn.fileuploader.languages['en'], {
            feedback: 'Drag & Drop',
            setting_asMain: 'Marcar como principal',
            setting_download: 'Download',
            setting_edit: 'Editar',
            setting_open: 'Abrir',
            setting_rename: 'Renomear',
            rename: 'Insira o novo nome:',
            renameError: 'Insira outro nome',
    ...
    $.post(url_site + 'galeria/examples/gallery/php/ajax.php?type=preload', { page_id: page_id }, function(result) {
