<?php
	...
	// mysqli connection
	$DB = mysqli_connect('localhost', 'super', 'Xw06si@dm', 'intranet');
	...
	if ($_action == 'upload') {
		...
		$FileUploader = new FileUploader('files', array(
			'limit' => 1,
			'fileMaxSize' => 20,
			'extensions' =>  ['image/*', 'jpg', 'png', 'gif', 'webp', 'jpeg', 'pdf/*', 'audio/*'],
		...
		//file_put_contents('C:/alaragon/www/site/log.txt', print_r($FileUploader, true).PHP_EOL, FILE_APPEND);
		...
		if (count($upload['files']) == 1) {
			...
			if (!$id)
				$query = $DB->query("INSERT INTO gallery(`page_id`,`title`, `file`, `type`, `size`, `index`, `date`) VALUES('$page_id','$title', '$file', '$type', '$size', 1 + (SELECT IFNULL((SELECT MAX(`index`) FROM gallery g), -1)), NOW())");
			else
	...
	if ($_action == 'preload') {
		...
		$query = $DB->query("SELECT * FROM gallery where page_id='$page_id' ORDER BY `index` ASC");
		...
		file_put_contents('C:/alaragon/www/site/log.txt', print_r($preloadedFiles, true).PHP_EOL, FILE_APPEND);
	...
	if ($_action == 'asmain') {
		if (isset($_POST['id']) && isset($_POST['name'])) {
			...
			$query = $DB->query("UPDATE gallery SET is_main = 0 WHERE page_id='$page_id'");
			$query = $DB->query("UPDATE gallery SET is_main = 1 WHERE id = '$id' AND page_id='$page_id'");
		}
		exit;
	}
